 ...
 * mesh
 */
class NeighborInfoModule : public ProtobufModule<meshtastic_NeighborInfo>,
                           private concurrency::OSThread {
  CallbackObserver<NeighborInfoModule, const meshtastic::Status *>
      nodeStatusObserver =
          CallbackObserver<NeighborInfoModule, const meshtastic::Status *>(
              this, &NeighborInfoModule::handleStatusUpdate);
...
protected:
  ...
  virtual bool handleReceivedProtobuf(const meshtastic_MeshPacket &mp,
                                      meshtastic_NeighborInfo *nb) override;
  ...
  */
  ...
  meshtastic_Neighbor *
  getOrCreateNeighbor(NodeNum originalSender, NodeNum n,
                      uint32_t node_broadcast_interval_secs, float snr);

  /*
   * Send info on our node's neighbors into the mesh
   */
  void sendNeighborInfo(NodeNum dest = NODENUM_BROADCAST,
  ...
  void alterReceivedProtobuf(meshtastic_MeshPacket &p,
                             meshtastic_NeighborInfo *n) override;
