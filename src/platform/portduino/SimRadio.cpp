...
void SimRadio::setTransmitDelay()
{
     ...
     *   This assumption is valid because of the offset generated by the radio to
...
void SimRadio::handleTransmitInterrupt()
{
    ...
    isReceiving = true;
    if (receivingPacket) // This happens when we don't consider something a
                         // collision if we weren't sending long enough
...
void SimRadio::onNotify(uint32_t notification)
{
    ...
    case TRANSMIT_DELAY_COMPLETED:
        if (receivingPacket) { // This happens when we had a timer pending and we
                               // started receiving
            handleReceiveInterrupt();
            startTransmitTimer();
            break;
        ...
        if (!txQueue.empty()) {
            if (!canSendImmediately()) {
                ...
                if (isChannelActive()) { // check if there is currently a LoRa packet on
                                         // the channel
                    ...
                    notifyLater(xmitMsec, ISR_TX,
                                false); // Model the time it is busy sending
                }
            }
        } else {
...
void SimRadio::startSend(meshtastic_MeshPacket *txp)
{
    ...
    if (p->decoded.payload.size <= sizeof(c.data.bytes)) {
        ...
        LOG_WARN("Payload size larger than compressed message allows! Send empty "
                 "payload");
    }
...
{
    // Simulator packet (=Compressed packet) is encapsulated in a MeshPacket, so
...
void SimRadio::startReceive(meshtastic_MeshPacket *p)
{
    ...
    notifyLater(airtimeMsec, ISR_RX,
                false); // Model the time it is busy receiving
#else
#if !MESHTASTIC_EXCLUDE_ERROR_TELEMETRY
    if (isActivelyReceiving()) {
        rxBad++;
...
void SimRadio::handleReceiveInterrupt()
{
    ...
    if (!isReceiving) {
        LOG_DEBUG("*** WAS_ASSERT *** handleReceiveInterrupt called when not in "
                  "receive mode");
        return;
    }
